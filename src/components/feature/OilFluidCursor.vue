...
fn main(@builtin(global_invocation_id) global_id : vec3<u32>) {
    ...
    // GRAYSCALE PALETTE - Pure black and white
    let t = uTime * 0.2 + p.x * 2.0 + p.y * 1.5;
    let col_start = palette(t, 
      vec3(1.0, 1.0, 1.0), // white base
      vec3(0.0, 0.0, 0.0), // no color variation
      vec3(1.0, 1.0, 1.0), // uniform frequency
      vec3(0.0, 0.0, 0.0)  // no phase shift
    );
...
fn fragment_main(fragData : VertexOut) -> @location(0) vec4<f32> {
    ...
    // Subtle chromatic aberration for oil-like shimmer
    let offset = 1.0;
    let id_r = u32(clamp(fuv.x + offset, 0.0, uGrid.dyeW - 1.0) + fuv.y * uGrid.dyeW);
    let id_b = u32(clamp(fuv.x - offset, 0.0, uGrid.dyeW - 1.0) + fuv.y * uGrid.dyeW);
    ...
    // Pure white output for difference blend mode
    let brightness = length(col);
    return vec4(vec3(1.0, 1.0, 1.0) * brightness * multiplier * 0.8, clamp(brightness * 0.6, 0.0, 0.9));
}
`;
...
const settings = {
  ...
  contain_fluid: false,
  vel_force: 0.35,
  vel_radius: 0.003, // Larger for thicker trail
  vel_diff: 0.98,
  dye_force: 0.55, // Increased for more visible color
  dye_radius: 0.006, // Larger radius for thicker oil trail
  dye_diff: 0.96, // Lower for faster fade-out
  vorticity: 0.12, // Moderate swirl
  pressure_iterations: 8,
};

onMounted(async () => {
  if (!navigator.gpu) {
    console.warn('WebGPU not supported - OilFluidCursor disabled');
    return;
  }
  
  const adapter = await navigator.gpu.requestAdapter();
  if (!adapter) {
    console.warn('WebGPU adapter not available - OilFluidCursor disabled');
    return;
  }
  ...
  // Use window dimensions directly for fullscreen
  ...
  // Force canvas to exact window dimensions
  ...
  const onMove = (e) => {
    ...
    // Normalized to 0..1 using full window dimensions
    mx = p.clientX / window.innerWidth;
    my = 1 - p.clientY / window.innerHeight;
  };
...
.fluid-cursor-canvas {
  ...
  mix-blend-mode: difference; /* Auto-inverts: white on dark, black on light */
